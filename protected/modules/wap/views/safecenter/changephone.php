...
<section class="box">
  ...
  <p class="form-head-p" style="padding-left:5%;">更换手机：<h3 id="xiugai-h3">￥<span id="xiugai-h3-span"></span></h3></p>
  <p class="form-head-p"><img src="<?php echo WAP_IMG_URL; ?>icon4.png">
  	手机号码：<?php echo $user_info->user_phone; ?>
  	<input type="hidden" id="user_phone" value="<?php echo $user_info->user_phone; ?>" />
  </p>
...
<script>
			$("#sendmsg").click(function(){
					$.post("<?php echo Yii::app()->controller->createUrl('site/sendsms'); ?>?type=safecenter_change_phone_1&phone="+$("#user_phone").val(),{},function(data){
			            var obj = eval("("+data+")");
			            if(obj.status == 1){
	                        window.sec = 60;
					        djs('sendmsg');

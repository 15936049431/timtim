<?php
    $this->page_title = '系统字段类别列表';
    $this->page_desc = '系统字段类别列表';
?>
...
<table class="table table-advance">
    <thead>
        <tr>
            ...
            <th><?php echo $model->getAttributeLabel('systemcat_alias'); ?></th>
            <th><?php echo $model->getAttributeLabel('systemcat_desc'); ?></th>
            <th><?php echo $model->getAttributeLabel('isdefault'); ?></th>
            <th style="width:100px">操作</th>
        </tr>
    </thead>
    <tbody>
        <?php foreach($systemcat_list as $k => $v){ ?>
        <tr>
            <td><input type="checkbox" class="checkdel" value="<?php echo $v->systemcat_id; ?>"></td>
            <td><?php echo $v->systemcat_name; ?></td>
            <td><?php echo $v->systemcat_alias; ?></td>
            <td><?php echo $v->systemcat_desc; ?></td>
            ...
            <td>
                <div class="btn-group">
                    ...
                    <a class="btn btn-small show-tooltip" title="" href="<?php echo Yii::app()->controller->createUrl('update',array('id'=>$v->systemcat_id)); ?>" data-original-title="编辑"><i class="icon-edit"></i></a>
                    <a class="btn btn-small btn-danger show-tooltip" title="" href="javascript:;" onclick="deldata('<?php echo $v->systemcat_id; ?>')" data-original-title="删除"><i class="icon-trash"></i></a>
                </div>
            </td>
        </tr>
        <?php } ?>

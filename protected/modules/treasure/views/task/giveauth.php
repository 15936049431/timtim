...
<!--<div class="row-fluid">
    ...
    <div class="span6">
        <div class="box box-green">
            <div class="box-title">
                <h3><i class="icon-table"></i>父任务</h3>
                <div class="box-tool">
                    <a data-action="collapse" href="#"><i class="icon-chevron-up"></i></a>
                    <a data-action="close" href="#"><i class="icon-remove"></i></a>
                </div>
            ...
            <div class="box-content">
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php foreach($parenttask_list as $k => $v){ ?>
                        <tr>
                            <td>1</td>
                            <td><?php echo $v['realname'] ?></td>
                            <td><a class="btn btn-danger btn-small" href="javascript:;" onclick="removeauth('<?php echo $v['parent'] ?>',2)"><i class="icon-trash"></i></a></td>
                        </tr>
                        <?php } ?>
                    </tbody>
                </table>
                ...
                <div class="control-group">
                    <div class="controls">
                        <select class="input-xlarge" name="select" id="select" data-rule-required="true" onchange="giveauth(this,this.value,2)" style="width:100%">
                            <option value="0">-- 添加父任务 --</option>
                            <?php foreach($apt_list as $k => $v){ ?>
                            <option value="<?php echo $v['name']; ?>"><?php echo $v['realname']; ?></option>
                            <?php } ?>
                        </select>
                    </div>
...
<script type="text/javascript">
    function giveauth(theobj,name,type){
        if(name!=0){
            if(confirm('确认执行此操作？')){
                $.post("<?php echo Yii::app()->controller->createUrl("task/ajaxgiveauth"); ?>",{
                    'Giveauth[cone]':name,
    ...
    function removeauth(name,type){
        if(confirm('确认执行此操作？')){
            $.post("<?php echo Yii::app()->controller->createUrl("task/ajaxremoveauth"); ?>",{

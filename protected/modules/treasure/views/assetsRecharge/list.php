<?php
    $this->page_title = '充值中心';
    $this->page_desc = '充值中心简介';
...
<div class="clearfix">
    <div class="pull-left" style="padding-left: 1em;">
        <?php $form = $this->beginWidget('CActiveForm',array(
            'action'=>Yii::app()->controller->createUrl('assetsRecharge/list'),
            'method'=>'GET',
            'htmlOptions'=>array(
                'name'=>'form_search'
            ),
        ...
        <table class="search_table">
            <tr>
                <td>状态：</td>
                <td><?php echo $form->dropDownList($model,'r_status',array('-1'=>'全部',"0"=>"未审","1"=>"通过","2"=>"失败"),array('class'=>'input-xlarge')); ?></td>
                <td>类型：</td>
                <td><?php echo $form->dropDownList($model,'r_type',array('-1'=>'全部',"1"=>"线上充值","2"=>"线下充值"),array('class'=>'input-xlarge')); ?></td>
				<td>充值方式：</td>
                <td><?php echo $form->dropDownList($model,'r_recharge_type',array_merge(array('0'=>'全部'),$recharge_list),array('class'=>'input-xlarge')); ?></td>
            </tr>
			<tr>
				<td>开始时间：</td>
...
<!--            <a class="btn btn-circle show-tooltip" title="" href="#" data-original-title="Export to PDF"><i class="icon-file-text-alt"></i></a>
        ...
        <div class="btn-group">
        	<a class="btn btn-circle show-tooltip" title="" target="_blank" href="<?php echo Yii::app()->controller->createUrl('list',array_merge($_GET,array('outfile_excel'=>'1'))); 
...
<th><?php echo $model->getAttributeLabel('r_type'); ?></th>
<th><?php echo $model->getAttributeLabel('r_recharge_type'); ?></th>
<th><?php echo $model->getAttributeLabel('r_status'); ?></th>
<th><?php echo $model->getAttributeLabel('r_id'); ?></th>
<th><?php echo $model->getAttributeLabel('r_money'); ?></th>
<th><?php echo $model->getAttributeLabel('r_addtime'); ?></th>
    ...
    <tbody>
        ...
        <tr>
            <!--<td><input type="checkbox" class="checkdel" value="<?php echo $v->r_id; ?>"></td>-->
<td><a href="<?php echo Yii::app()->controller->createUrl('user/update',array('id'=>$v->r_user_id)); ?>" ><?php echo $v->user->user_name; ?></a></td>
<td><?php echo $v->user->real_name; ?></td>
<td><?php echo $model->itemAlias('r_type',$v->r_type); ?></td>
<td><?php echo LYCommon::GetItem($v->r_recharge_type,($v->r_type==1)?'assets_recharge_type':'assets_oevrrecharge_type'); ?></td>
<td><?php echo $model->itemAlias('r_status',$v->r_status); ?></td>
...
<td><?php echo LYCommon::subtime($v->r_addtime,2); ?></td>
            <td>
                <div class="btn-group">
                    ...
                    <a class="btn btn-small show-tooltip" title="" href="<?php echo Yii::app()->controller->createUrl('update',array('id'=>$v->r_id)); ?>" data-original-title="编辑"><i class="icon-edit"></i></a>
                    <!--<a class="btn btn-small btn-danger show-tooltip" title="" href="javascript:;" onclick="deldata('<?php echo $v->r_id; ?>')" data-original-title="删除"><i class="icon-trash"></i></a>-->
                </div>
            </td>
        </tr>
        <?php } ?>

...
class SiteController extends WController {
    ...
    public function actionIndex() {
    	...
    	$banner_list = $link_model->findAll(array('limit' => '2', 'condition' => 'link_type=2', 'order' => 'link_order ASC'));
    ...
    public function actionRegister() {
        ...
        if (isset($_POST['Register'])) {
            ...
            if ($model->validate()) {
                ...
                $code_info = $code_model->findByAttributes(array('target' => $model->user_phone, 'codecat_id' => '7'));
                if (!empty($code_info)) {
                    if ($code_info->exc_time > time() && $code_info->status == 0 && $code_info->error_num < 3) {
                        if ($model->code == $code_info->code) {
                            ...
                            $transacton = Yii::app()->db->beginTransaction();
                            try {
                                if ($model->insert()) {
                                    //修改手机验证码为可用状态
                                    $code_info->status = 1;
                                ...
                                $transacton->commit();
                                unset(Yii::app()->session['send_phone_yes']);

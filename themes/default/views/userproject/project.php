<?php
    ...
    $this -> page_title = '我的借款';
?>
				<div class="user_money_main">
                        <dl class="user_money_tab clear">
                            <dd class="sel"><a href="<?php echo Yii::app()->controller->createUrl("userproject/project"); ?>">我的借款</a></dd>
						...
						<?php $form = $this->beginWidget('CActiveForm',array(
							'method'=>'GET',
							'action'=>Yii::app()->controller->createUrl('userproject/project'),
							'htmlOptions'=>array(
								'name'=>'form_search',
								'class'=>'user_search',
							),
...
<!--                        <span>查询类型：</span>
                        ...
                        <table class="user_recharge_record ba">
							...
							<tr>
                                ...
                                <td><a target="_blank" href="<?php echo Yii::app()->controller->createUrl('agreement/invest',array('pid'=>$v->p_id)); ?>">协议书</a></td>
                                ...
                                <td>
                                    <?php if($v->p_status==0){ ?><a href="#" attr="<?php echo $v->p_id; ?>" id="getback">撤回</a>
                                    <?php }elseif($v->p_status==1){ ?><?php echo ($v->p_account>=$v->p_account_yes)?"满标待审中":"正在招标中"; ?>
                                    <?php }elseif($v->p_status==2){ ?><a href="#" attr="<?php echo $v->p_id; ?>" id="getback">撤回</a>
                                    <?php }elseif($v->p_status==3){ ?>正在还款中
                                    <?php }elseif($v->p_status==4){ ?><a href="#" attr="<?php echo $v->p_id; ?>" id="getback">撤回</a>
                                    <?php }elseif($v->p_status==5){ ?><a href="#" attr="<?php echo $v->p_id; ?>" id="getback">撤回</a>
                                    <?php }elseif($v->p_status==6){ ?>已撤回
                                    <?php }elseif($v->p_status==7){ ?>正常结束<?php } ?>
                                </td>
							</tr>
							<?php } ?>
                            <tr>

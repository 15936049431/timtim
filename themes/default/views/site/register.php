    ...
    <?php $form = $this -> beginWidget('CActiveForm',array(
             ...
             'validateOnChange'=>true,
                ...
                <table class="fl">
                    ...
                    <tr>
                        <td>
                            <?php echo $form->textField($model,'user_name',array('class'=>'ba uname')); ?>
                            <span class="error" style="display:none;"><?php echo $form->error($model,"user_name"); ?></span>
                            <span class="input_desc">请输入用户名</span>
	                        <span class="Validform_checktip">请输入4-20位字母、数字或中文，不含特殊字符。</span>
                        </td>
                    </tr>
                    <tr>
                        <td>
                    ...
                    <tr>
                        <td>
                            <?php echo $form->textField($model,'authcode',array('class'=>'ba code')); ?>
                            <span class="error" style="display:none;"><?php echo $form->error($model,"authcode"); ?></span>
                            <span class="input_desc">请输入验证码</span>
                    ...
                    <tr>
                        <td>
                        	<?php echo $form->passwordField($model,'login_pass',array('class'=>'ba pwd')); ?>
                        	<span class="error" style="display:none;"><?php echo $form->error($model,"login_pass"); ?></span>
                    ...
                    <tr>
                        <td>
                        	<?php echo $form->passwordField($model,"re_login_pass",array("class"=>"ba pwd")); ?>
                        	<span class="error" style="display:none;"><?php echo $form->error($model,"re_login_pass"); ?></span>
                    ...
                    <tr>
                        <td>
                            <input type="checkbox" id="agree" checked="checked" disabled="disabled"/>
                            <label for="agree">我已阅读并且同意皖平财富的<a target="_blank" href="<?php echo SITE_UPLOAD."register.doc"?>">使用协议</a></label>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <input type="submit" value="立即注册" class="btn fl" />                            
                        </td>
                    </tr>
                </table>
	<?php $this -> endWidget(); ?>
...
$(function () {
    ...
    $("#sendMessage").click(function(){
		if(!(/^1[0-9]{10}$/.test($("#Register_user_phone").val()))){ 
			pointererror('不是完整的11位手机号或者正确的手机号前七位', 6);
		}else if($("#code_auth").val()=="null" || $("#code_auth").val()==""){
			pointererror('图形验证码不能为空', 6);
		}else{
            $.post("<?php echo Yii::app()->controller->createUrl("site/sendphone"); ?>",{"phone":$("#Register_user_phone").val(),"authcode":$("#code_auth").val(),"type":"register_phone"},function(result){
            	 var obj = eval("("+result+")");
            	 if(obj.status == 1){
